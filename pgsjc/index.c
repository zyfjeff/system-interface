 ...
 *
 *  nohup和shell内置的disown　会将SIGHUP信号的处理设置为SIG_IGN，从而避免在关闭终端后导致后台进程结束
 *
 *
 ...
 *
 *  如果SIGTSTP信号被捕获，那么就不会执行默认的停止进程的动作，可以在SIGTSTP的处理器中生成一个SIGSTOP信号来解决这个问题
 *  最切当的方式是在SIGTSTP信号处理器中再生成一个SIGTSTP信号来停止进程。
 *
 *  孤儿进程组:如果一个进程组中至少有一个成员拥有一个位于同一会话但不同进程组中的父进程，就不是孤儿进程组
 *  会话首进程位于孤儿进程组中。
 *
 *  如果一个进程组变成了孤儿进程组并且拥有很多已停止执行的成员，那么系统会向进程组中的所有成员发送一个SIGHUP信号通知
 *  他们已经与会话断开连接了，之后再发送一个SIGCONT信号确保他们恢复执行，如果孤儿进程组不包含被停止的成员，那么就不会
 *  发送任何信号
 *
 *  孤儿进程组对于read/write调用时，内核返回EIO错误,而不是发送SIGTIN或SIGTTOU信号
 *
 */
