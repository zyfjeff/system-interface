...
处于原始模式下的终端并不意味着原始的，未经过解释的控制字符会传递给shell，或者伪终端从设备的其他任何前台进程组,
也不代表该进程组的输出会以原始方式传递给用户的终端，相反，是在从设备中对终端特殊字符做解释(除非该从设备也被显
式地设置为原始模式).通过将用户终端设为原始模式，我们可以避免对输入输出字符做两轮解释
